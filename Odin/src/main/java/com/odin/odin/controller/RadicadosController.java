...
public class RadicadosController {
    ...
    public ResponseEntity<?> agregarObservacion(@PathVariable Long id,
        ...
        if (comentario.isBlank()) {
            return ResponseEntity.badRequest().body(Map.of("error", "El comentario es obligatorio"));
        }

        Long usuario = actor != null ? actor : numeroLong(payload.get("usuario"));
        Observaciones item = Observaciones.builder()
                .id_radicado(id)
                .id_usuario(usuario)
    ...
    public Radicados create(@RequestBody Radicados radicado) {
        ...
        registrar(id(saved), usuario(saved), "radicacion", "Radicado creado");
        return saved;
    }
    ...
    public ResponseEntity<?> asignar(@PathVariable Long id,
                                     @RequestBody Map<String, Object> payload,
                                     @RequestHeader(value = "X-User-Id", required = false) Long actor) {
    ...
    public ResponseEntity<?> reasignar(@PathVariable Long id,
                                       @RequestBody Map<String, Object> payload,
                                       @RequestHeader(value = "X-User-Id", required = false) Long actor) {
        ...
        return radicadosRepository.findById(id).map(r -> {
            Integer anterior = r.getId_usuario();
            int updated = radicadosRepository.actualizarAsignacion(id, nuevo, dependencia);
    ...
    public ResponseEntity<?> cerrar(@PathVariable Long id,
        ...
        return radicadosRepository.findById(id).map(r -> {
            ...
            if (estado == null || estado <= 0) estado = 3;
            r.setId_estado(estado);
            r.setFecha_cierre(LocalDateTime.now().toString());
            r.setId_usuario_cierre((actor != null ? actor : usuario(r)).intValue());
            Radicados saved = radicadosRepository.save(r);
            ...
            if (!observacion.isBlank()) {
                observacionesRepository.save(Observaciones.builder()
                        .id_radicado(id).id_usuario(usuarioCierre).comentario(observacion)
    ...
    private void registrar(Long radicado, Long usuario, String accion, String descripcion) {
        // La tabla de Supabase exige id_usuario NOT NULL y FK válida.
        if (usuario == null || !usuariosRepository.existsById(usuario)) return;
        historialRepository.save(HistorialRadicado.builder()
    ...
    private boolean puedeModificar(Long usuarioId) {
        // Mientras no exista autenticación integrada, se permite compatibilidad de UI.
        ...
        return usuariosRepository.findById(usuarioId)
                ...
                .map(rol -> rolPermitido(rol.getRol()) || rolPermitido(rol.getNombre()))
                .orElse(false);
    }

    private boolean rolPermitido(String rol) {
        if (rol == null) return false;
        String valor = rol.toLowerCase();
        return valor.contains("admin") || valor.contains("coordin") || valor.contains("gestor")
                || valor.contains("oper") || valor.contains("recep");
    }

    private Long usuario(Radicados r) {
        return r.getId_usuario() == null ? null : r.getId_usuario().longValue();
    }

    private Long usuario(Radicados r, Long fallback) {
        return usuario(r) == null ? fallback : usuario(r);
    }

    private Long id(Radicados r) { return r.getId_radicado(); }

    private Integer numero(Object value) {
        if (value == null) return null;
        try { return value instanceof Number ? ((Number) value).intValue() : Integer.valueOf(value.toString().trim()); }
        catch (Exception e) { return null; }
    }

    private Long numeroLong(Object value) {
        if (value == null) return null;
        try { return value instanceof Number ? ((Number) value).longValue() : Long.valueOf(value.toString().trim()); }
        catch (Exception e) { return null; }
    }

    private String texto(Object value) { return value == null ? "" : value.toString().trim(); }

    private ResponseEntity<Map<String, String>> prohibido(String accion) {
        return ResponseEntity.status(403).body(Map.of("error", "El rol no tiene permiso para " + accion));
    }
}
