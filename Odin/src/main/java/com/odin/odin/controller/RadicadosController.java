...
public class RadicadosController {
    ...
    public ResponseEntity<?> agregarObservacion(@PathVariable Long id, @RequestBody Map<String,Object> payload) {
        ...
        Observaciones item = Observaciones.builder().id_radicado(id).id_usuario(numero(payload.get("usuario")))
                .comentario(comentario).fecha(LocalDateTime.now().toString()).build();
        Observaciones saved = observacionesRepository.save(item);
        registrar(id, item.getId_usuario(), "observacion", comentario);
    ...
    public Radicados create(@RequestBody Radicados radicado) {
        ...
        registrar(saved.getId_radicado(), saved.getId_usuario(), "radicado", "Radicado creado");
        return saved;
    }
    ...
    public ResponseEntity<Radicados> update(@PathVariable Long id, @RequestBody Radicados radicado) {
        return radicadosRepository.findById(id).map(existing -> {
            ...
            Radicados saved = radicadosRepository.save(radicado);
            registrar(id, saved.getId_usuario(), "modificacion", "Radicado actualizado");
            return ResponseEntity.ok(saved);
    ...
    public ResponseEntity<?> cambiarEstado(@PathVariable Long id, @RequestBody Map<String,Integer> payload,
                                            @RequestHeader(value="X-User-Id", required=false) Integer actor) {
        if (!puedeModificar(actor)) return ResponseEntity.status(403).body(Map.of("error", "El rol no tiene permiso para cambiar estados"));
        Integer nuevo = payload.get("estado");
        ...
        return radicadosRepository.findById(id).map(r -> {
            Integer anterior = r.getId_estado(); r.setId_estado(nuevo); Radicados saved = radicadosRepository.save(r);
            registrar(id, actor != null ? actor : r.getId_usuario(), "cambio_estado", "Estado " + anterior + " → " + nuevo);
            return ResponseEntity.ok(saved);
        }).orElse(ResponseEntity.notFound().build());
    }

    @PatchMapping("/{id}/asignar")
    public ResponseEntity<?> asignar(@PathVariable Long id, @RequestBody Map<String,Integer> payload,
                                     @RequestHeader(value="X-User-Id", required=false) Integer actor) {
        if (!puedeModificar(actor)) return ResponseEntity.status(403).body(Map.of("error", "El rol no tiene permiso para asignar"));
        Integer usuario = payload.get("usuario");
        if (usuario == null || usuario <= 0) return ResponseEntity.badRequest().body(Map.of("error", "Usuario inválido"));
        return radicadosRepository.findById(id).map(r -> {
            Integer anterior = r.getId_usuario(); r.setId_usuario(usuario); Radicados saved = radicadosRepository.save(r);
            registrar(id, actor != null ? actor : usuario, "asignacion", "Responsable " + anterior + " → " + usuario);
            return ResponseEntity.ok(saved);
        }).orElse(ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<?> reasignar(@PathVariable Long id, @RequestBody Map<String,Object> payload,
                                       @RequestHeader(value="X-User-Id", required=false) Integer actor) {
        if (!puedeModificar(actor)) return ResponseEntity.status(403).body(Map.of("error", "El rol no tiene permiso para reasignar"));
        Integer usuarioNuevo = numero(payload.get("usuarioNuevo")); Integer dependenciaNueva = numero(payload.get("dependenciaNueva"));
        ...
        return radicadosRepository.findById(id).map(r -> {
            ...
            int updated = radicadosRepository.actualizarAsignacion(id, usuarioNuevo, dependenciaNueva);
            if (updated == 0) return ResponseEntity.internalServerError().body(Map.of("error", "No se actualizó la asignación"));
            Reasignaciones re = new Reasignaciones(); re.setId_radicado(id.intValue()); re.setId_usuario_anterior(anterior);
            re.setId_usuario_nuevo(usuarioNuevo); re.setId_dependencia_nueva(dependenciaNueva); re.setFecha(LocalDateTime.now().toString());
            reasignacionesRepository.save(re);
            registrar(id, actor != null ? actor : usuarioNuevo, "reasignacion", "Responsable " + anterior + " → " + usuarioNuevo);
    ...
    public ResponseEntity<?> cerrar(@PathVariable Long id, @RequestBody(required=false) Map<String,Object> payload,
        ...
        return radicadosRepository.findById(id).map(r -> {
            ...
            r.setId_estado(estado); r.setFecha_cierre(LocalDateTime.now().toString()); r.setId_usuario_cierre(actor != null ? actor : r.getId_usuario());
            Radicados saved = radicadosRepository.save(r);
            String detalle = texto(payload == null ? null : payload.get("observacion"));
            registrar(id, r.getId_usuario_cierre(), "cierre", detalle.isBlank() ? "Radicado cerrado" : detalle);
            if (!detalle.isBlank()) observacionesRepository.save(Observaciones.builder().id_radicado(id).id_usuario(r.getId_usuario_cierre()).comentario(detalle).fecha(LocalDateTime.now().toString()).build());
            return ResponseEntity.ok(saved);
        }).orElse(ResponseEntity.notFound().build());
    }

    @GetMapping("/{id}/permisos")
    public ResponseEntity<Map<String,Object>> permisos(@PathVariable Long id, @RequestParam Integer usuario) {
        if (!radicadosRepository.existsById(id)) return ResponseEntity.notFound().build();
    ...
    private void registrar(Long radicado, Integer usuario, String accion, String descripcion) {
        historialRepository.save(HistorialRadicado.builder().id_radicado(radicado).id_usuario(usuario).accion(accion)
                .descripcion(descripcion).fecha(LocalDateTime.now().toString()).build());
    }

    private boolean puedeModificar(Integer usuarioId) {
        if (usuarioId == null) return true;
        return rolesRepository.findById((long) usuarioId).map(Roles::getRol).map(this::rolPermitido).orElse(true);
    }

    private boolean rolPermitido(String rol) {
        if (rol == null) return false; String r = rol.toLowerCase();
        return r.contains("admin") || r.contains("coordin") || r.contains("gestor") || r.contains("oper") || r.contains("recep");
    }

    private Integer numero(Object value) { if (value == null) return null; try { return value instanceof Number ? ((Number)value).intValue() : Integer.valueOf(value.toString()); } catch (Exception e) { return null; } }
