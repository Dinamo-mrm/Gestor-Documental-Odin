...
public interface RadicadosRepository
        extends JpaRepository<Radicados, Long> {

    @Query(
            value = "SELECT * FROM radicados " +
                    "ORDER BY id_radicado DESC LIMIT 5",
            nativeQuery = true
    )
    List<Radicados> findTop5UltimosRadicados();

    @Query(
            "SELECT COUNT(r) FROM Radicados r " +
                    "WHERE r.id_estado = 1"
    )
    ...
    @Query(
            value = "SELECT COUNT(*) FROM radicados " +
    ...
    @Query(
            value = "SELECT * FROM radicados " +
                    ...
                    "AND NULLIF(TRIM(fecha_vencimiento), '')::date < CURRENT_DATE " +
                    "AND id_estado NOT IN (3,4) " +
                    "ORDER BY NULLIF(TRIM(fecha_vencimiento), '')::date ASC",
            nativeQuery = true
    )
    ...
    @Query(
            value = "SELECT * FROM radicados " +
                    "WHERE NULLIF(TRIM(fecha_vencimiento), '') IS NOT NULL " +
                    "AND NULLIF(TRIM(fecha_vencimiento), '')::date " +
                    "BETWEEN CURRENT_DATE " +
                    "AND (CURRENT_DATE + CAST(:dias AS integer)) " +
                    "AND id_estado NOT IN (3,4) " +
                    "ORDER BY NULLIF(TRIM(fecha_vencimiento), '')::date ASC",
            nativeQuery = true
    )
    List<Radicados> findProximosAVencer(
    ...
    @Transactional
    @Query(
            value = "UPDATE radicados " +
                    "SET id_usuario = :usuario, " +
                    "id_dependencia = :dependencia " +
                    "WHERE id_radicado = :radicado",
            nativeQuery = true
    )
    int actualizarAsignacion(
            @Param("radicado") Long radicado,
