...
SELECT setval('radicados_numero_seq', greatest(coalesce((SELECT max((regexp_match(numero_radicado, '^RAD-[0-9]{4}-([0-9]+)
...
COMMIT;
))[1]::bigint) FROM radicados WHERE numero_radicado ~ '^RAD-[0-9]{4}-[0-9]+
    ON radicados (id_estado, id_dependencia, id_tramite);
